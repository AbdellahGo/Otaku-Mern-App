...
const SeriesCard = ({ id, title, image, status, chapters, lastReadChapter }: Props) => {
            ...
            <Link to={`/otaku-details/${id}`} key={id} className="block">
                ...
                <div className="mt-[12px]">
                    ...
                    <div className="flex flex-wrap items-start gap-[10px] gap-y-[10px] mt-[15px]">
                        ...
                        <p className={`${pStyles}`}>Chapters: <span className="text-lightSkyBlue">{chapters}</span></p>
                        <p className={`${pStyles}`}>Last Read: <span className="text-lightSkyBlue">chapter {lastReadChapter}</span></p>
                    </div>
                </div>
            </Link>
        </div>
